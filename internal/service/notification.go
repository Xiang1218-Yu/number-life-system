...
func (s *NotificationService) createOnce(userID uint, kind string, referenceID uint, title, message string, due time.Time) (bool, error) {
	...
	row := domain.Notification{UserID: userID, Type: kind, ReferenceID: referenceID, Title: title, Message: fmt.Sprintf("%s [记录:%d]", message, referenceID), DueAt: due, Channel: "console", Status: "pending"}
